...
- Layout container: `max-w-5xl mx-auto px-4` — used in header, footer, and `<main>` to keep everything aligned. Adjust the max-width per project as needed. `src/main.css` sets `html { font-size: 125% }` (20px rem base, same as the old adaltavoce site), so every Tailwind size — text, spacing, and these `max-w-*` containers (`max-w-6xl` = 1440px rather than 1152px) — renders 25% larger than Tailwind's defaults; don't compensate with px values or you'll fight it. The header is `h-20` with `gap-8` between nav items for the same reason. Two header knobs are deliberately in one place each: the mobile-menu breakpoint is a custom Tailwind breakpoint, `--breakpoint-nav` in `src/main.css`'s `@theme` (default `40rem` = 800px at the 125% root), used in `header.php` as the `nav:` variant (`nav:flex`/`nav:hidden`) instead of hard-coded `sm:` — Tailwind can't build class names from a PHP variable, so a named breakpoint is what keeps it to a single edit; and the logo height is the `$logoClass` variable at the top of `header.php` (default `h-12 w-auto`).
...
- Custom **Child pages** block (`site/blueprints/blocks/child-pages.yml` + `site/snippets/blocks/child-pages.php`): pick a page, render its `children()->listed()` as a card grid. It's registered through the global `'blocks.fieldsets'` option in `site/config/config.php` (which must also re-list the core blocks, because setting it replaces Kirby's default list) rather than a `fieldsets:` list on each blueprint field. Each card shows the child's `pageExcerpt` under the title when set, and an image at `aspect-[3/2]` chosen as `cardImage` → else `heroBackgroundImage` (only if `heroToggle` is on and `heroBackgroundType` is `image`, since those fields are hidden in the Panel otherwise) → else none. The whole card is a single `<a>` so the image is clickable without nested links, and `group-hover:scale-105` on the image gives the hover zoom; the image is `alt=""` since the title is the link text. The block is wrapped in `not-prose` to escape `default.php`'s `.prose` styling. See README's "Custom blocks: Child pages".
